        ...
        <div className="max-w-7xl mx-auto" ref={topRef}>
          ...
          {isSearchActive && (
            <p className="text-center text-gray-600 text-sm mb-8">
              Showing {filtered.length} result{filtered.length !== 1 && 's'} for "<span className="font-medium">{search}</span>"
            </p>
          )}
          ...
          <AnimatePresence mode="wait">
            {pageReady && (
              <motion.div
                key={`${currentPage}-${search}-${category}-${sort}`}
              ...
              {Array.from({ length: totalPages }, (_, i) => (
                <button
                  ...
                  className={`px-4 py-1 rounded-md text-sm font-medium border transition ${
                    currentPage === i + 1
                      ? "bg-[#83b735] text-white border-[#83b735]"
